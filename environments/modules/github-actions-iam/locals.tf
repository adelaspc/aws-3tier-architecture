locals {
  ...
  app_deploy_role_name      = "${local.name_prefix}-github-app-deploy"
  app_deploy_policy_name    = "${local.name_prefix}-github-app-deploy"
  ...
  app_deploy_role_arn      = "arn:${data.aws_partition.current.partition}:iam::${data.aws_caller_identity.current.account_id}:role/${local.app_deploy_role_name}"
  app_deploy_policy_arn    = "arn:${data.aws_partition.current.partition}:iam::${data.aws_caller_identity.current.account_id}:policy/${local.app_deploy_policy_name}"
